...
def all_k3tog(width_repeats, row, height_repeat): {
    // does all of the k3togs
    valley_section = bool(height_repeat % 2);
    
    for repeat in range(width_repeats): {
            
            // right leaning
            k3tog(triple_index = row + 4 + (2 * repeat * size), is_right = True, do_valley = valley_section);
            
            // left leaning
            k3tog(triple_index = ((repeat + 1) * size * 2) - 5 - row, is_right = False, do_valley = valley_section);
...
def knit_row(width_repeats, row, height_repeat, direc, is_front, do_tuck): {
    // knits a whole row
...
with Carrier as fabric_carr: {
    ...
    // main body
    for height_repeat in range(height_repeats): {
        for row in range(height): {
            
            // set up knit togethers with xfers
            all_k3tog(width_repeats, row, height_repeat);

            // knit row
            knit_row(width_repeats, row, height_repeat, Leftward, is_front = True, do_tuck = True);

            // do some xfers for gardering
            garder_xfer(width_repeats, current_bed_is_front = True);

            // purl row
            knit_row(width_repeats, row, height_repeat, Rightward, is_front = False, do_tuck = False);
